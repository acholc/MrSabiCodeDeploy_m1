                ...
                <div class="container">
                    <div class="row">
                        <div class="page-title-left">
                            <h6 class="page-title-heading mr-0 mr-r-5">Listing Review</h6>
                        </div>
                        <!-- /.page-title-left -->
                        <div class="page-title-right d-none d-sm-inline-flex align-items-center">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active">Listing Review</li>
                            </ol>
                        </div>
                        <!-- /.page-title-right -->
                    </div>
							...
							<div class="widget-body clearfix">
								<div class="table-responsive">
									<table class="table table-striped" data-toggle="datatables" data-plugin-options='{"searching": false}'>
										...
										<tbody>	
										@if(isset($data['listing_review']))
								       @php $i=1; @endphp
								       @foreach($data['listing_review'] as $reviews)
											<tr class="user_row_{{ $reviews['id'] }}">
												<td>{{ $i }}</td>
												<td>@if($reviews['user_info']){{ $reviews['user_info']['name'] }}@endif</td>
												<td>@if($reviews['listing_info']){{ $reviews['listing_info']['title']  }}@endif</td>
	...
	$('.lnr-trash').on('click',function(){
     ...
     $('.yes_delete').on('click',function(){
              ...
              $.ajax({       
                  ...
                  url:"{{route('adm.delete_listing_reviews')}}",
                  data: {'id':id},                 
                  cache: false, 
